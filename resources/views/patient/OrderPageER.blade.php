    ...
    <div class="bg-white rounded-lg shadow-md w-full max-w-4xl">
        ...
        <div class="p-6">
            ...
            <div class="mb-4">
                ...
                <p class="bg-gray-50 text-sm text-gray-600 py-2 px-4 rounded-md w-full">{{ $erorder->title }}</p>
            </div>

            <!-- Order Type -->
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Order Type</label>
                <p class="bg-gray-50 text-sm text-gray-600 py-2 px-4 rounded-md w-full">{{ $erorder->type }}</p>
            </div>

            <!-- Order Description -->
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Description</label>
                <p class="bg-gray-50 text-sm text-gray-600 py-2 px-4 rounded-md w-full">{{ $erorder->description }}</p>
            </div>

            <!-- Order Status -->
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Order Status</label>
                <p class="bg-gray-50 text-sm text-gray-600 py-2 px-4 rounded-md w-full">{{ $erorder->order_status }}</p>
            </div>

            <!-- Order Status -->
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Ordered By</label>
                <p class="bg-gray-50 text-sm text-gray-600 py-2 px-4 rounded-md w-full">{{ $erorder->admittingDoctor->name }} - {{ $erorder->admittingDoctor->profile->specialization }}</p>
            </div>

            <!-- Order Status -->
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Admission Status</label>
                <p class="bg-gray-50 text-sm text-gray-600 py-2 px-4 rounded-md w-full">{{ $erorder->status }}</p>
            </div>
             ...
             <div class="mb-4">
                ...
                @php
                    $orderDate = new DateTime($erorder->order_date); // Convert string to DateTime object
                @endphp
            ...
            <div>
                ...
                <button id="myBtn{{ $erorder->id }}" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50" data-order-id="{{ $erorder->id }}">Open QR</button>
                ...
                <div id="myModal{{ $erorder->id }}" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 overflow-auto">
                    ...
                    <div class="relative bg-white rounded-lg mx-auto mt-20 p-6 w-11/12 md:w-1/3">
                        ...
                        <div class="mb-4 text-center"> <!-- Added text-center class -->
                            ...
                            <!-- QR Code Image -->
                            <img id="qrCodeImg{{ $erorder->id }}" src="" alt="QR Code" class="mx-auto"> <!-- Added mx-auto class -->
                        </div>     
                        <!-- Print Button -->
                        <div class="text-center">
                            <button onclick="printQRCode('{{ $erorder->id }}')" class="py-1 px-3 border border-gray-400 text-gray-700 bg-gray-200 rounded hover:bg-gray-300 focus:outline-none focus:ring-1 focus:ring-gray-300">
                                Print
                            </button>
                        </div>                 
                    </div>
...
function openModal(modalId, patientRecordId) {
    ...
    if (modal) {
        ...
        if (qrCodeImg) {
            ...
            fetch(`/patient/erorder/qr-code/${patientRecordId}`) // Update the endpoint URL as necessary
